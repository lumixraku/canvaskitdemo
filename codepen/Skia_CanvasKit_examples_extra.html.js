/*
 * Source: https://github.com/google/skia/blob/main/modules/canvaskit/npm_build/extra.html
 */
...
const loadDrinksJSON = fetch(cdn + "drinks.json").then((response) =>
    response.text()
);
const loadConfettiJSON = fetch(cdn + "confetti.json").then((response) =>
    response.text()
);
const loadOnboardingJSON = fetch(cdn + "onboarding.json").then((response) =>
    response.text()
);
const loadMultiframeJSON = fetch(
    cdn + "skottie_sample_multiframe.json"
).then((response) => response.text());

const loadFlightGif = fetch(cdn + "flightAnim.gif").then((response) =>
    response.arrayBuffer()
);
const loadSkp = fetch(cdn + "picture2.skp").then((response) =>
    response.arrayBuffer()
);
const loadFont = fetch(cdn + "Roboto-Regular.ttf").then((response) =>
    response.arrayBuffer()
);
const loadDog = fetch(cdn + "dog.jpg").then((response) =>
    response.arrayBuffer()
);
const loadMandrill = fetch(cdn + "mandrill_256.png").then((response) =>
    response.arrayBuffer()
);
const loadBrickTex = fetch(cdn + "brickwork-texture.jpg").then((response) =>
    response.arrayBuffer()
);
const loadBrickBump = fetch(cdn + "brickwork_normal-map.jpg").then((response) =>
    response.arrayBuffer()
);
...
Promise.all([ckLoaded, loadMultiframeJSON, loadFlightGif]).then(
    ([ck, jsonstr, gif]) => {
        SkottieExample(ck, "sk_animated_gif", jsonstr, fullBounds, {
            "image_0.png": gif
        });
    }
);
...
Promise.all([ckLoaded, loadSkp]).then((results) => {
    SkpExample(...results);
});
...
function ParagraphAPI2(CanvasKit, fontData) {
    ...
    function keyhandler(e) {
        ...
        if (e.ctrlKey) {
            switch (e.key) {
                case "r":
                    editor.applyStyleToSelection({ color: [1, 0, 0, 1] });
                    return;
                case "g":
                    editor.applyStyleToSelection({ color: [0, 0.6, 0, 1] });
                    return;
                case "u":
                    editor.applyStyleToSelection({ color: [0, 0, 1, 1] });
                    return;
                case "k":
                    editor.applyStyleToSelection({ color: [0, 0, 0, 1] });
                    return;

                case "i":
                    editor.applyStyleToSelection({ italic: "toggle" });
                    return;
                case "b":
                    editor.applyStyleToSelection({ bold: "toggle" });
                    return;
                case "l":
                    editor.applyStyleToSelection({ underline: "toggle" });
                    return;

                case "]":
                    editor.applyStyleToSelection({ size_add: 1 });
                    return;
                case "[":
                    editor.applyStyleToSelection({ size_add: -1 });
                    return;
...
Promise.all([ckLoaded, loadDog, loadMandrill]).then((values) => {
    ...
    const draw = (x, y, shader) => {
        ...
        canvas.drawRect(
            CanvasKit.LTRBRect(0, 0, quadrantSize, quadrantSize),
            paint
        );
        canvas.restore();
        paint.delete();
    ...
    const getBlurrySpiralShader = (rad_scale) => {
        ...
        const spiralShader = spiralEffect.makeShader(
            [rad_scale, quadrantSize / 2, quadrantSize / 2, 1, 1, 1, 1, 0, 0, 0, 1],
            true
        );
    ...
    const drawFrame = () => {
        surface.requestAnimationFrame(drawFrame);
        const thresholdShader = getBlurrySpiralShader(
            Math.sin(Date.now() / 5000) / 2
        );
...
function GlyphGame(canvas, robotoData) {
    ...
    function drawFrame(canvas) {
        ...
        canvas.concat(
            CanvasKit.M44.setupCamera(
                CanvasKit.LTRBRect(0, 0, sizeX, sizeY),
                halfDim,
                cam
            )
        );
        ...
        for (const letter of letters) {
            ...
            distanceFromPointer = CanvasKit.Vector.dist(
                pointer,
                rotationPoint.slice(0, 2)
            );
            ...
            canvas.concat(
                CanvasKit.M44.multiply(
                    ...
                    CanvasKit.M44.translated(
                        CanvasKit.Vector.mulScalar(rotationPoint, -1)
                    )
                )
            );
